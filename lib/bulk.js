...
(function() {
  ...
  Bulk = class Bulk {
    ...
    constructor(workingPath, presetId) {
      ...
      debuglog(`[constructor] Bulk id: ${this.id}, cluster.worker.id:${cluster.worker && cluster.worker.id}, @pathToFile:`, this.pathToFile);
    ...
    commit(clichouseClient, statement) {
      ...
      debuglog(`${this} [commit] go committing, count:${this.count}, pathToFile:${this.pathToFile}`);
      theOutputStream = this.outputStream;
      theOutputStream.end((err) => {
        var bulkDataStream, dbStream;
        if (err != null) {
          debuglog(`${this} [commit] FAIL to end stream. error:`, err);
          this._committing = false; //unlock
          return;
        }
        bulkDataStream = fs.createReadStream(this.pathToFile);
        ...
        dbStream.on('finish', () => {
          debuglog(`${this} [commit] success dbStream:finish`);
          this._committing = false;
          this._committed = true;
