...
(function() {
  var CLUSTER_WORKER_ID, Cargo, CargoOptions, REG_INVALID_SQL_TABLE_NAME_CHAR, TABLE_NAME_TO_CARGO, assert, cargoOptionToHttpOption, cluster, createCargo, debuglog, err, examCargos, examOneCargo, fs, http, https, init, os, path, pathToConfig, profileConfig, profileName;
  ...
  init = function(config) {
    var commitInterval, isToFlushBeforeCrash, maxInsetParts, maxRows, maxTime;
    ...
    if (maxInsetParts > 0) {
      CargoOptions.maxInsetParts = maxInsetParts;
    }
    delete config.maxInsetParts;
    //debuglog "[init] CargoOptions:", CargoOptions
    isToFlushBeforeCrash = config.saveWhenCrash === false ? false : true;
  ...
  examOneCargo = async function(cargo) {
    var err, examStartAt, msSpent;
    try {
      ...
      if (msSpent > 100) {
        debuglog(`[examOneCargo] ${cargo.tableName} takes: ${msSpent}ms`);
      }
    } catch (error) {
      err = error;
  ...
  examCargos = async function() {
    var cargo, msSpent, routineStartAt, tableName;
    //debuglog "[examCargos]"
...
//await eachSeries(TABLE_NAME_TO_CARGO, examOneCargo)
    ...
    if (msSpent < 1000) {
      ...
      await new Promise((resolve) => {
        return setTimeout(resolve, 1000 - msSpent);
      });
    }
