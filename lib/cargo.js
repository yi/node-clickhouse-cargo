...
(function() {
  var CLUSTER_WORKER_ID, Cargo, DEFAULT_COMMIT_INTERVAL, EXTNAME_UNCOMMITTED, FILENAME_PREFIX, MAX_COMMIT_PER_EXAM_ROUTINE, MIN_ROWS, MIN_TIME, NOOP, StaticCountWithinProcess, assert, cluster, crypto, debuglog, fs, fsAsync, got, isThisLeader, os, path, pipeline, promisify, qs, stream;
  ...
  Cargo = class Cargo {
    ...
    async commitToClickhouseDB() {
      var err, filenamList, filepath, httpPostOptions, j, len, res, rotationPrefix;
...
// submit each local uncommit sequentially
      for (j = 0, len = filenamList.length; j < len; j++) {
        ...
        httpPostOptions = cargoOptionToHttpOption(CargoOptions, {
          path: "/?" + qs.stringify({
