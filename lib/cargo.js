...
(function() {
  var CLUSTER_WORKER_ID, Cargo, DEFAULT_COMMIT_INTERVAL, EXTNAME_UNCOMMITTED, FILENAME_PREFIX, MAX_COMMIT_PER_EXAM_ROUTINE, MIN_ROWS, MIN_TIME, NOOP, StaticCountWithinProcess, assert, cargoOptionToHttpOption, cluster, debuglog, fs, fsAsync, isThisLeader, os, path, promisify, qs, url;
  ...
  Cargo = class Cargo {
    ...
    async flushToFile(forced) {
      ...
      if (!(forced || (this.cachedRows.length > this.maxRows) || (Date.now() > this.lastFlushAt + this.maxTime))) {
        debuglog(`${this} [flushToFile] SKIP threshold not reach`);
        return;
      }
    ...
    async exam() {
      ...
      if (!(Date.now() > this.lastCommitAt + this.commitInterval)) {
        debuglog(`[exam ${this.tableName}] SKIP tick not reach`);
        return;
      }
      if (!isThisLeader()) {
    ...
    async commitToClickhouseDB() {
      var err, filenamList, filepath, j, len, res, rotationPrefix;
      ...
      debuglog(`[commitToClickhouseDB] 3 filenamList(${filenamList.length})`);
