...
(function() {
  var CLUSTER_WORKER_ID, Cargo, DEFAULT_COMMIT_INTERVAL, DEFAULT_MAX_INSET_PARTS, EXTNAME_UNCOMMITTED, FILENAME_PREFIX, MAX_COMMIT_PER_EXAM_ROUTINE, MIN_ROWS, MIN_TIME, MultiStream, NOOP, StaticCountWithinProcess, assert, cargoOptionToHttpOption, cluster, debuglog, fs, fsAsync, isThisLeader, os, path, promisify, qs, url;
  ...
  Cargo = class Cargo { // extends EventEmitter
    ...
    async uploadLocalUncommitsToClickhouse(filenamList) {
      var filepath, j, len, proc;
      debuglog("[uploadLocalUncommitsToClickhouse] filenamList:", filenamList && filenamList.length);
      ...
      proc = (resolve, reject) => {
        ...
        req = this.vehicle.request(this.httpPostOptions, (res) => {
          ...
          debuglog(`[uploadLocalUncommitsToClickhouse] SUCCESS uploaded ${filenamList.length} uncommits, x-clickhouse-query-id:`, res.headers && res.headers['x-clickhouse-query-id'], ", x-clickhouse-summary:", res.headers && res.headers['x-clickhouse-summary']);
          resolve(res);
        });
        req.on('error', function(err) {
        ...
        req.write(this.statement);
        streams = filenamList.map(function(filepath) {
    ...
    async commitToClickhouseDB() {
      var err, filenamList, rotationPrefix, splitMark;
      try {
      ...
      this.uploadLocalUncommitsToClickhouse(filenamList);
    }
