...
(function() {
  var CLUSTER_WORKER_ID, Cargo, DEFAULT_COMMIT_INTERVAL, EXTNAME_UNCOMMITTED, FILENAME_PREFIX, MAX_COMMIT_PER_EXAM_ROUTINE, MIN_ROWS, MIN_TIME, NOOP, StaticCountWithinProcess, assert, cluster, crypto, debuglog, detectLeaderWorker, fs, os, path, toSQLDateString;
  ...
  Cargo = class Cargo {
    ...
    flushToFile(callbak = NOOP) {
      ...
      if (!(this.cachedRows.length > 0)) {
        debuglog(`${this} [flushToFile] nothing to flush`);
        this.lastFlushAt = Date.now();
        callbak();
        return;
      }
