...
(function() {
  var CLUSTER_WORKER_ID, Cargo, DEFAULT_COMMIT_INTERVAL, DEFAULT_MAX_INSET_PARTS, EXTNAME_UNCOMMITTED, FILENAME_PREFIX, MAX_COMMIT_PER_EXAM_ROUTINE, MIN_ROWS, MIN_TIME, NOOP, StaticCountWithinProcess, assert, cargoOptionToHttpOption, cluster, debuglog, fs, fsAsync, isThisLeader, os, path, promisify, qs, url;
  ...
  Cargo = class Cargo { // extends EventEmitter
    ...
    async uploadLocalUncommitsToClickhouse(filenamList) {
      ...
      proc = (resolve, reject) => {
        var err, filepath, j, len, req, srcStream;
        req = this.vehicle.request(this.httpPostOptions, (res) => {
          if (!(res && res.statusCode === 200)) {
            reject(new Error(`ClickHouse server response statusCode:${res && res.statusCode}`));
            return;
